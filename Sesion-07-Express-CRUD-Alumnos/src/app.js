 ...
 * @type {import('express').RequestHandler}
 */
export function validarAlumno(req, res, next) {const { nombre, apellido, email, edad } = req.body || {};
    ...
    if (!email || typeof email !== 'string' || !email.trim() || !email.includes('@')) {
        return res.status(400).json({ error: 'El email debe ser válido y contener @' });
    }
...
export function crearApp(repositorio) {
    ...
    app.post('/alumnos', autenticacionFalsa, validarAlumno, (req, res) => {
        ...
        const nuevo = repositorio.crear({ 
            nombre: nombre.trim(),
            apellido: apellido.trim(),
            email: email.trim(), 
            ...(edad !== undefined && edad !== null && edad !== '' ? { edad: Number(edad) } : {})     
           });
    ...
    const { nombre, apellido, email, edad } = req.body;
    const actualizado = repositorio.actualizar(req.params.id, {
      nombre: nombre.trim(),
      apellido: apellido.trim(),
      email: email.trim(),
      ...(edad !== undefined && edad !== null && edad !== '' ? { edad: Number(edad) } : {})    });
